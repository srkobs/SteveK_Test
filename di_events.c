...
static void (*inputEvtFn[NUM_OF_INPUT_EVENTS - START_IE])(void) = \
{
	...
	setDispenserState,		//IE_DISABLE_DISPENSE
...
void motorTest(){
	...
	P2DIR |= BIT5;
	P2OUT |= BIT5;
	_delay_cycles(6000000);	//500ms delay
	P2OUT &= ~BIT5;
	P2DIR = dirTemp;
	WDTCTL = WDTPW + wdtTemp + WDTCNTCL;
}
...
void i2cTest(){
	...
	//Switches pins to outputs, pulls them low and then releases them
	P1DIR |= BIT6 + BIT7;
	P1OUT &= ~(BIT6 + BIT7);
	_delay_cycles(6000000);	//500ms delay
	P1DIR = p1dirTemp;
