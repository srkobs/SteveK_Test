...
void printCapData(uint32_t baseCnt, uint32_t measCnt)
{
	printf("%n\t%n\t%l\n", baseCnt, measCnt, (measCnt - baseCnt));
	/*
	//Send base count, lower 16 bits 1 byte at a time
	sendByte ((uint8_t)(baseCnt >> 6));
	sendByte ((uint8_t)((baseCnt & 0x00FFFFFF) >> 4));
	sendByte ((uint8_t)((baseCnt & 0x0000FFFF) >> 2));
	sendByte ((uint8_t)((baseCnt & 0x000000FF));

	//Send raw count, lower 16 bits 1 byte at a time
	sendByte ((uint8_t)(measCnt >> 6));
	sendByte ((uint8_t)((measCnt & 0x00FFFFFF) >> 4));
	sendByte ((uint8_t)((measCnt & 0x0000FFFF) >> 2));
	sendByte ((uint8_t)((measCnt & 0x000000FF));
	*/
}
